...
# =====================================================================
# 4. DEEP ALNS BATCH GENERATOR (DESTROY-AND-RECREATE & DEEP OPT)
# =====================================================================
function generate_deep_alns_batch!(P_batch, current_p, elite_list, batch_size, N, stagnation_counter)
    n_elites = length(elite_list)
    is_deep_escape = stagnation_counter > 1000
...
function run_deep_solver()
    ...
    println("\nStarting Deep ALNS Optimization Run...")
    start_time = time()

    for epoch in 1:max_epochs
        elite_list = collect(elite_basins)
        generate_deep_alns_batch!(P_host, current_p, elite_list, batch_size, N, stagnation_counter)
        copyto!(P_gpu, P_host)
