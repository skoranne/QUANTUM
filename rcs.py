...
# 2. Export to OpenQASM string (without measurement operations)
concrete_kernel = cudaq.synthesize(
    ibm_uchicago_interleaved_rcs, 
    total_qubits,   # Arg 1 (97)
    depth,      # Arg 2 (example)
    host_data_map,
    host_ancilla_pairs_flat,
    doping_mask
...
# 2. Define the exact native basis gates for the IBM Heron processor
# Heron uses Controlled-Z (cz) instead of CNOT (cx) for two-qubit operations
heron_basis_gates = ["cz", "rz", "sx", "x", "id"]

# 3. Optimize the kernel using the Heron basis set
# This forces CUDA-Q's compiler passes to decompose/transpile gates (like H or CX) into Heron equivalents
qasm_string = cudaq.translate(concrete_kernel, format="qir")
