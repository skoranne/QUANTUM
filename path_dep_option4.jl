# ===================================================================
# RIGOROUS MINSPM-QAE vs. EXPLICIT QAE KERNEL & FINANCIAL VALIDATION
# Decouples Monte Carlo Ground Truth from Spectral Spectrum Extraction.
# ===================================================================
...
# Fast Quantile LUT for Heavy Monte Carlo Ground Truth
...
# 1. Heavyweight Financial Validation (1M Path Monte Carlo Ground Truth)
function run_heavy_monte_carlo_ground_truth(n_paths::Int, steps::Int, strike::Float64, bound_b::Float64, r::Float64, T::Float64)
    ...
    Threads.@threads for path in 1:n_paths
        ...
        for _ in 1:steps
            ...
            log_s1 += fast_quantile(lut_axa, u1) - axa_p[3]
            log_s2 += fast_quantile(lut_mch, u2) - mch_p[3]
    ...
    se = std(payoffs) / sqrt(n_paths)
    return mean_amp, mean_amp * bound_b * exp(-r * T), se * bound_b * exp(-r * T)
end

# 2. Explicit QAE Spectral Evaluation O(Q)
function run_explicit_qae_spectrum(theta_true::Float64, m_bits::Int)
    ...
    max_val, best_y = findmax(probs)
    return best_y - 1, elapsed, probs
end

# 3. MINSPM-QAE Compressed Spectral Evaluation O(u)
function run_minspm_compressed_spectrum(theta_true::Float64, m_bits::Int, u_classes::Int)
    Q = Int64(1) << m_bits
    q_float = Float64(Q)
    compressed_y = zeros(Int, u_classes)
    compressed_p = zeros(Float64, u_classes)
    ...
    elapsed = time() - t_start
    return best_y, elapsed, compressed_y, compressed_p
end

function execute_experimental_benchmark()
    println(repeat("=", 95))
    println(" EXPERIMENTAL PARADIGM: EXPLICIT QAE vs. MINSPM-QAE + MC GROUND TRUTH")
    println(repeat("=", 95))
    ...
    T = 1.0
    steps = 12
    n_paths = 1000000  # 1M MC paths for ground-truth confidence interval
    ...
    m_bits = 20  # Q = 1,048,576 states for explicit evaluation
    u_classes = 4096  # Compressed equivalence classes for MINSPM
    theta_true = asin(sqrt(clamp(mc_amp, 1e-12, 1.0 - 1e-12)))
    
    println("[2] Executing Explicit QAE Spectrum (Q = 2^20 states)...")
    exp_best_y, exp_time, exp_probs = run_explicit_qae_spectrum(theta_true, m_bits)
    exp_est_a = sin(pi * Float64(exp_best_y) / Float64(1 << m_bits))^2
    exp_price = exp_est_a * bound_b * exp(-r * T)
    
    println("[3] Executing MINSPM-QAE Compressed Spectrum (u = 4,096 classes)...")
    min_best_y, min_time, min_y_grid, min_p_grid = run_minspm_compressed_spectrum(theta_true, m_bits, u_classes)
    min_est_a = sin(pi * Float64(min_best_y) / Float64(1 << m_bits))^2
    min_price = min_est_a * bound_b * exp(-r * T)
    
    # Compute L_infinity error over peak location and reconstructed price
    ...
    @printf("%-26s | %-16d | %-16d | Peak Match    | y_opt Match\n", "Peak Location (y_opt)", exp_best_y, min_best_y)
