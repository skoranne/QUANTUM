...
function multi_opt_perturbation!(p::Vector{Int32}, N::Int, rng::AbstractRNG, strength::Int)
    """Apply multiple random 3-opt moves with increasing strength"""
    for _ in 1:strength
        idx = rand(rng, 1:(N-2))
...
function diversify_population!(P_host::Matrix{Int32}, elite_seed::Vector{Int32}, 
    ...
    """
    Diversify population by:
    1. Keeping some elite copies with increased perturbation
    2. Creating diverse solutions from scratch
    3. Mixing solutions
    """
    elite_copies = div(batch_size, 16)  # 6.25% pure elite
    diverse_copies = div(batch_size, 4)  # 25% heavily perturbed
    crossover_copies = div(batch_size, 4)  # 25% crossover-based
    
    # Tier 1: Pure elite (keep structure)
    for b in 1:elite_copies
        p = copy(elite_seed)
        multi_opt_perturbation!(p, N, rng, 2)  # Light perturbation
        P_host[:, b] .= p
    end
    
    # Tier 2: Heavily diversified (break structure)
    for b in (elite_copies+1):(elite_copies+diverse_copies)
        p = copy(elite_seed)
        strength = 10 + div(gen, 100)  # Increase perturbation strength over time
        multi_opt_perturbation!(p, N, rng, strength)
        P_host[:, b] .= p
    end
    
    # Tier 3: Random shuffles with elite backbone
    for b in (elite_copies+diverse_copies+1):(elite_copies+diverse_copies+crossover_copies)
        p = collect(Int32, 1:N)
        shuffle!(rng, p)
        # Preserve ~20% of elite structure
        for i in 1:div(N, 5)
...
function run_exhaustive_basin_sweeper_v2()
    ...
    println("Executing Improved 3-Opt Basin Probing with Adaptive Diversification...")
    println("Initial best cost: $global_best_cost\n")

    for gen in 1:max_generations
        # Decide: diversify or perturb
        should_diversify = no_improve_count > max_no_improve
        ...
        if should_diversify
            ...
            # Apply normal perturbation to all population members
            P_host_new = copy(P_host)
            
            for b in 1:batch_size
                # Adaptive perturbation strength
                strength = 1 + div(no_improve_count, 50)  # Increase strength when stuck
        ...
        # Evaluate new/diversified population on GPU
        copyto!(P_gpu, P_host)
        ...
        # Update population and track improvements
        improved_count = 0
        for b in 1:batch_size
            if E_host_new[b] < E_host[b]
                ...
                if E_host[b] < global_best_cost
                    ...
                    no_improve_count = 0
                    println("Generation $gen | NEW BEST: $global_best_cost (Improved in batch: $improved_count)")
                end
            end
        end
        ...
        if gen % 200 == 0 || gen == 1
            ...
            println("Generation $gen | Best: $global_best_cost | Mean: $(round(mean_cost, digits=0)) | Worst: $(round(worst_cost, digits=0)) | No-Improve: $no_improve_count")
        end
    end

    integer_cost = Int(round(global_best_cost))

    println("\n--- Final Basin Sweep Result ---")
    println("Final Cost: $integer_cost")
    println("Known best: 44759294")
    improvement = 44759294 - integer_cost
    if improvement > 0
        println("✓ NEW RECORD! Improvement: $improvement below known best")
    elseif improvement == 0
        println("= Matched known best record")
    else
        println("~ Within $(abs(improvement)) of known best")
    end
    println("Best Permutation Vector (first 50 elements):")
    println(join(global_best_perm[1:50], " "))

    open("solution_improved.txt", "w") do io
        println(io, N)
    ...
    println("\nSuccessfully saved to solution_improved.txt")

    CUDA.reclaim()
end

run_exhaustive_basin_sweeper_v2()
