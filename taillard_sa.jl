...
function run_minspm_simulated_annealing()
    ...
    println("Initializing Taillard tai256c with MINSPM Simulated Annealing Optimizer...")
    ...
    best_permutation = copy(P_host[:, best_idx])
    
    T = 150000.0f0
    cooling_rate = 0.985f0

    println("Running Parallel GPU Search via Host-Managed Annealing Loop...")
    for iter in 1:num_iterations
        P_host_new = copy(P_host)
        for b in 1:batch_size
            idx = rand(rng, 1:(N-1))
            P_host_new[idx, b], P_host_new[idx+1, b] = P_host_new[idx+1, b], P_host_new[idx, b]
        end
        
        # Use copyto! instead of broadcast assignment to safely transfer host data to device
